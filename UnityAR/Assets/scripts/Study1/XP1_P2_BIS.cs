...
public class XP1_P2_BIS : MonoBehaviour
{
	...
	void GaugeInit(float duration)
	{
		...
		for (int i = 0; i < expanDialSticks.NbRows; i++)
		{
			for (int j = 0; j < expanDialSticks.NbColumns; j++)
			{
				...
				if (i == (int)gaugePosition.x && j == (int)gaugePosition.y)
				{
					expanDialSticks.modelMatrix[i, j].TargetPlaneTexture = "trackerZone";
					expanDialSticks.modelMatrix[i, j].TargetPlaneOffset = Vector2.zero;
	...
	void triggerChangeXYAxis(sbyte deltaPosX, sbyte deltaPosY)
	{
		Vector2 gaugePosition = gaugePositions[gaugeIndex];

		expanDialSticks.modelMatrix[(int)gaugePosition.x, (int)gaugePosition.y].TargetAxisX += deltaPosX;
		expanDialSticks.modelMatrix[(int)gaugePosition.x, (int)gaugePosition.y].TargetAxisY += deltaPosY;
		float coeffAxisX = expanDialSticks.modelMatrix[(int)gaugePosition.x, (int)gaugePosition.y].CurrentAxisX / 127.0f;
		float coeffAxisY = expanDialSticks.modelMatrix[(int)gaugePosition.x, (int)gaugePosition.y].CurrentAxisY / 127.0f;
		
		float coeffOffsetX = Mathf.PI/2f * -coeffAxisX;
		float coeffOffsetY = Mathf.PI/2f * coeffAxisY;
		//Debug.Log(coeffOffsetX + " " + coeffOffsetY);
		expanDialSticks.modelMatrix[(int)gaugePosition.x, (int)gaugePosition.y].TargetPlaneOffset = new Vector2(coeffOffsetX, coeffOffsetY);

		expanDialSticks.modelMatrix[(int)gaugePosition.x, (int)gaugePosition.y].TargetShapeChangeDuration += 0.1f;
		expanDialSticks.triggerShapeChange();
		expanDialSticks.modelMatrix[(int)gaugePosition.x, (int)gaugePosition.y].TargetTextureChangeDuration += 0.1f;
		expanDialSticks.triggerTextureChange();
	}
	void Update()
	{
